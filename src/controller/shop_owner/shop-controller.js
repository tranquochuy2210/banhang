const{Shop, User, Item, Follow}=require('../../models/index')
const db=require('../../config/db')
const {upload}=require('../utils/add_file')
const { Sequelize } = require('../../config/db')
const fs=require('fs')
const path=require('path')

const uploadShopImage=upload.single('shop')
const getAddShop=async(req,res)=>{
    res.render('my_shop/add-shop')
}

const addShop=async(req,res)=>{
    try{
        const userId=req.user.id
        const {name,address}=req.body
        const isExist=await Shop.findOne({where:{name}})
        if(isExist){
            return res.json({success:false,message:'tên đã tồn tại'})
        }
        const image=req.file?`/uploads/shop/${req.file.filename}`:null
        const shop=await Shop.create({name,userId,address,image})
        res.json({success:true,message:'tạo thành công'})
    }catch(error){
        res.json({success:false,message:error.message})
    }
}
const getAllShop=async(req,res)=>{
    try{
        let shops=await Shop.findAll()
        res.status(200).json({shops})
    }catch(error){
        console.log(error)
    }
}
const getShop=async(req,res)=>{
    try{
        let shop =await Shop.findOne({
            where:{id:req.params.id},
            include:[{model:User},{model:Item},{model:Follow}],
        })
        if(!req.user || !shop.Follows.some((follow)=>follow.userId===req.user.id)){
            shop.followed=false
        }else{
            shop.followed=true
        }
        console.log(shop.Items)
        const follows=await Follow.findAll({where:{shopId:req.params.id},attributes:[[Sequelize.fn('COUNT','userId'),'followCount']]})
        shop.followCount=follows[0].dataValues.followCount
        res.render('my_shop/shop',{shop})
    }catch(error){
        console.log(error)
    }
}
const editShop=async(req,res)=>{
    try{
        const {name,address,reOpenOn}=req.body
        const status=req.body.status?true:false
        if(req.file){
            const link=path.join(__dirname,`../../public${req.shop.image}`)
            fs.unlink(link , (err) => {
                if (err) {
                    console.log('cannot find')
                };
                console.log('image was deleted');
              })
        }
        const image=req.file?`/uploads/shop/${req.file.filename}`:req.shop.image
        await req.shop.update({name,address,status,reOpenOn,image})
        res.status(200).json({success:true,message:'cập nhật thành công'})
    }catch(error){
        console.log(error)
    }
}
const deleteShop=async(req,res)=>{
    try{
        const id=req.params.id
        const shop=await Shop.findByPk(id)
        if(!shop){
            return res.json({error:'unable to find shop'})
        }
        await shop.destroy()
        res.json({msg:'Shop has been deleted'})
    }catch(error){
        console.log(error)
    }
}

const getMyShop=async(req,res)=>{
    try{
        res.render('my_shop/my-shop',{shop:req.shop})
    }catch(error){
        console.log(error)
    }
}
const getAllItems=async(req,res)=>{
    try{
        const shopId=req.shop.id
        const items=await Item.findAll({where:{shopId}})
        res.render('my_shop/item/items',{items,shop:req.shop})
    }catch(error){
        console.log(error)
    }
}


const getAllOrders = async (req,res)=>{
    try{

        const page = Number(req.query.page)
        const skip = (page-1)* 5
        const [result,metadata]=await db.query(`
        SELECT a.id,a.status,d.lastName as userName,b.itemId, b.quantity,c.name,a.deliveryDate FROM shopee.orders as a
        inner join order_items as b on b.orderId=a.id
        inner join items as c on c.id=b.itemId
        inner join users as d on d.id=a.userId
        where c.shopId=${req.shop.id} limit 5 offset ${skip}`)
        const more = result.length<5 ? false:true
        if(page === 1){
            res.render('my_shop/order/orders',{orders:result, more})
        }else {
            res.json({orders:result, more })
        }
        
    }catch(error){
        console.log(error)
    }
}


const getDeletedOrders=async(req,res)=>{
    try{
        const [result,metadata]=await db.query(`
        SELECT a.id,a.status,d.lastName as userName, b.quantity,c.name,a.deliveryDate FROM shopee.orders as a
        inner join order_items as b on b.orderId=a.id
        inner join items as c on c.id=b.itemId
        inner join users as d on d.id=a.userId
        where c.shopId=${req.shop.id} and a.status='deleted'`)
        res.render('my_shop/deleted-order',{orders:result})
    }catch(error){
        console.log(error)
    }
}

const getRevenue=async(req,res)=>{

}
const getRating=async(req,res)=>{
    try{
        const [result,metadata]=await db.query(
            `select c.lastName,a.comment,a.rating,DATE_FORMAT(a.createdAt, "%M %e %Y") as createdAt,b.name as ItemName from evaluates as a
            inner join items as b on b.id=a.itemId
            inner join users as c on a.userId=c.id
            where b.shopId=${req.shop.id}`)
            let rating ={}
            result.forEach((evaluate)=>{
                if(rating[evaluate.rating]){
                    rating[evaluate.rating]+=1
                }else{
                    rating[evaluate.rating]=1
                }
            })
            let sum=Object.keys(rating).reduce((total,current)=>total+Number(current)*rating[current],0)
            let count=Object.keys(rating).reduce((total,current)=>total+Number(rating[current]),0)
            let average=Math.floor(sum/count*10)/10
            res.render('my_shop/rating/ratings',{average,rating,ratingItems:result})
    }catch(error){
        console.log(error)
    }
}



module.exports={uploadShopImage,getAddShop,addShop,getAllShop,getShop,editShop,deleteShop,getMyShop,getAllItems,getAllOrders,getDeletedOrders,getRevenue,getRating}